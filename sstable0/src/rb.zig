...
pub const Tree = struct {
    root: ?*Node,
    compareFn: fn (*Node, *Node) Order, //Compare,
    ...
    pub fn replace(tree: *Tree, old: *Node, newconst: *Node) !void {
        ...
        if (tree.compareFn(old, new) != Order.eq) return ReplaceError.NotEqual;
...
fn doLookup(key: *Node, tree: *Tree, pparent: *?*Node, is_left: *bool) ?*Node {
    ...
    while (maybe_node) |node| {
        var res: Order = tree.compareFn(node, key);
        if (res == Order.eq) {
            return node;
        }
        pparent.* = node;
